...
class Tagger:
    def __init__(
        self, topdir=None, auth=None, database=None, limit=None, interval=None, tasks=1
    ):
        self.topdir = topdir
        self.auth = auth
        self.limit = limit
    ...
    def image_is_in_database(self, image):
        res = self.curs.execute("select 1 from photos where path = ?", (image,))
        ...
        with aiohttp.MultipartWriter("form-data") as root:
            with image.open("rb") as fd:
                ...
                async with self.session.post(
                    "https://api.imagga.com/v2/tags", data=root
                ) as res:
    ...
    def store(self, job):
        ...
        self.curs.execute("insert into photos (path) values (?)", (str(image),))
    ...
    async def run(self):
        ...
        pl = pl | pipe.map(self.tag, task_limit=self.tasks) | pipe.map(self.store)
...
def main(
    loglevel, api_key, api_secret, credentials, database, limit, interval, tasks, topdir
):
